...
void describe("x4edPack", () => {
	void it("should return the correct output", async (t) => {
		await t.test("0", () => {
			deepEqual(
				x4edPack(0),
				[10, 165, 242, 211, 128, 70, 92, 272, 42, 35, 297, 151, 138, 93, 270]
			);
		});
		...
		await t.test("1", () => {
			deepEqual(
				x4edPack(1),
				[285, 348, 159, 313, 213, 103, 249, 36, 219, 167, 16, 190, 133, 63, 263]
			);
		});
		...
		await t.test("2", () => {
			deepEqual(
				x4edPack(2),
				[290, 71, 122, 247, 89, 238, 22, 218, 356, 145, 241, 41, 177, 206, 152]
			);
		});
		...
		await t.test("3", () => {
			deepEqual(
				x4edPack(3),
				[352, 302, 139, 295, 255, 47, 145, 69, 277, 7, 213, 103, 249, 36, 16]
			);
		});
		...
		await t.test("4", () => {
			deepEqual(
				x4edPack(4),
				[333, 311, 341, 295, 166, 86, 164, 109, 286, 14, 255, 47, 196, 148, 7]
			);
		});
...
void describe("atqPack", () => {
	void it("should return the correct output", async (t) => {
		await t.test("0", () => {
			deepEqual(atqPack(0), [80, 79, 22, 27, 41, 15, 77, 63]);
		});

		await t.test("1", () => {
			deepEqual(atqPack(1), [23, 7, 82, 13, 65, 7, 59, 68]);
		});

		await t.test("2", () => {
			deepEqual(atqPack(2), [3, 12, 83, 52, 75, 12, 55, 34]);
		});

		await t.test("3", () => {
			deepEqual(atqPack(3), [84, 65, 7, 65, 7, 80, 72, 11]);
		});

		await t.test("4", () => {
			deepEqual(atqPack(4), [32, 22, 49, 13, 85, 67, 72, 25]);
		});

		// Lowest mode 1 seed.
		await t.test("8388608", () => {
			deepEqual(atqPack(8388608), [40, 61, 27, 8, 2, 49, 60, 3]);
		});
	});
});
